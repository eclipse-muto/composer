...
class LaunchDescriptionStructure:
        ...
        self.launch_description = launch.LaunchDescription(
            [
                launch.actions.IncludeLaunchDescription(
                    AnyLaunchDescriptionSource(self.launch_file_path),
                ),
            ]
        )
    ...
    def get_launch_description_data(self):
        ...
        context._set_asyncio_loop(asyncio_loop)

        self._provide_required_launch_arguments(context, self.launch_description.entities)
        ...
        for entity in entities:
            if isinstance(entity, launch.actions.DeclareLaunchArgument):
                if entity.default_value is None:
                    ...
                    context.launch_configurations[entity.name] = entity.default_value.perform(context)
    ...
    def _get_entity_data(self, entity, context):
        ...
        if isinstance(entity, launch.actions.DeclareLaunchArgument):
            data = {
                ...
                'name': entity.name,
                'default_value': self._resolve_substitution(entity.default_value, context) if entity.default_value else None,
            ...
            # If launch arguments are provided, collect them
            ...
            # Visit the entity to load the included launch description
            entity.visit(context)
            sub_entities = entity.describe_sub_entities()
            if sub_entities:
                data['included_entities'] = self._get_entities_data(sub_entities, context)
        elif isinstance(entity, launch_ros.actions.Node):
            package = self._resolve_substitution(getattr(entity, '_Node__package', None), context)
            executable = self._resolve_substitution(getattr(entity, '_Node__node_executable', None), context)
            node_name = self._resolve_substitution(getattr(entity, '_Node__node_name', None), context)
            node_namespace = self._resolve_substitution(getattr(entity, '_Node__node_namespace', None), context)
            parameters = self._resolve_parameters(getattr(entity, '_Node__parameters', None), context)
        ...
        elif isinstance(entity, launch.actions.SetEnvironmentVariable):
            data = {
                'type': 'SetEnvironmentVariable',
                'name': self._resolve_substitution(entity.name, context),
                'value': self._resolve_substitution(entity.value, context),
            }
        elif isinstance(entity, launch.actions.ExecuteProcess):
            cmd = [self._resolve_substitution(arg, context) for arg in entity.process_details['cmd']]
            data = {
                'type': 'ExecuteProcess',
                'cmd': cmd,
                'name': self._resolve_substitution(entity.process_details.get('name'), context),
                'env': entity.process_details.get('env'),
                'additional_attributes': entity.process_details,
            }
        else:
